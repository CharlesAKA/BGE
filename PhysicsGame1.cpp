...
float lower_limit = -1.5f;
float uper_limit = 1.5f;
float x = 5.0f;
float z = 0.0f;
bool PhysicsGame1::Initialise()
{
	...
	shared_ptr<PhysicsController> body = physicsFactory->CreateCapsule(4, 5, glm::vec3(x, 15, z), glm::angleAxis(90.0f, glm::vec3(1, 0, 0)));//Build a quaternion from an angle and an axis.
	...
	btFixedConstraint * neck_to_head = new btFixedConstraint(*head->rigidBody, *neck->rigidBody, LocalA, LocalB);
	dynamicsWorld->addConstraint(neck_to_head);
	LocalC.setIdentity();
	LocalD.setIdentity();
	...
	btFixedConstraint * neck_to_body = new btFixedConstraint(*neck->rigidBody, *body->rigidBody, LocalC, LocalD);
	dynamicsWorld->addConstraint(neck_to_body);

	//front legs and shoulder joints
	...
	shoulderjoint->setLimit(lower_limit, uper_limit);
	...
	shoulderjoint2->setLimit(lower_limit, uper_limit);//hinge rotation limit
	...
	shoulderjoint3->setLimit(lower_limit, uper_limit);//hinge rotation limit
	...
	shoulder4->setLimit(lower_limit, uper_limit);//hinge rotation limit
	...
	btHingeConstraint * dog_tail = new btHingeConstraint(*tail->rigidBody, *body->rigidBody, btVector3(0, 3, 0), btVector3(0, -12, 0), btVector3(1, 1, 1), btVector3(0, 1, 0), false);
	dynamicsWorld->addConstraint(dog_tail);
	if (!Game::Initialise()) {
		return false;
	}
